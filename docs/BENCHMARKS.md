...
- Put latency is dominated by the synced Pebble batch commit (`pebble.Sync`
  ...
  Group-commit batching across concurrent writers is the obvious
...
- Allocation tuning (sync.Pool etc.) was not applied: profiles do not show
  hot-path allocation pressure at these throughputs, so optimization is
  deferred until profiles justify it.
